...
The rendered banner is only a screen signal, so a park record opens only when `quota-axi` reports the window the banner names (five-hour, or the seven-day row for a weekly banner) with at most `FM_LIMIT_PARK_CORROBORATE_MAX_PCT` remaining; an unreadable `quota-axi` still admits the park, and a refusal writes nothing, so the next sweep opens a real park as soon as the live window agrees.
...
FM_LIMIT_PARK_CORROBORATE_MAX_PCT=5 # quota-axi percentRemaining at or below which the window the banner names corroborates it, so a park record may be opened; an unreadable quota-axi or a missing row still admits the park (bin/fm-limit-park-lib.sh)

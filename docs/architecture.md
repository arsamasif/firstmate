...
A Claude worker parked on the account's usage-limit banner is a declared external wait of the same class, recorded durably and resumed by a tokenless owner scheduled outside the primary after the window resets; [`watcher-continuity.md`](watcher-continuity.md#usage-limit-outage) owns that contract and `bin/fm-limit-resume.sh` owns the mechanics.

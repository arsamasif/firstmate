...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, Windows/MSYS `CLAUDE_PID` identity, and holder liveness) for fm-lock.sh and the Claude Stop auto-arm |

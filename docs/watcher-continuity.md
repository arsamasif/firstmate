...
The durable wake queue already preserves every actionable record across the outage; what the park adds is the truthful description: the sweep writes `state/.limit-park-outage` when it sees the primary parked or a worker parked behind a stale beacon, and `bin/fm-guard.sh` describes a stale beacon inside that window as parked on the usage limit from one time to another, instead of alarming about a lapsed watcher, until a healthy watcher is seen again or the window plus its grace has passed.

...
A Claude primary and every Claude worker share one account usage window, so when it is exhausted they park within the same minute on Claude Code's rendered banner, and the exit-2 rewake the Stop hook delivers for a pending wake needs a model turn the limit refuses.
Until 2026-09-04 that gap let the chain lapse silently: the watcher cycle ended with an actionable wake, the auto-arm recorded a rewake, no turn ran, nothing re-armed for 7.9 hours, and the next session read the stale beacon as a lapsed watcher.
The contract now names that outage as its own state rather than a wedge or a lapse.
`bin/fm-composer-lib.sh` owns the banner shape, `bin/fm-limit-park-lib.sh` owns the park record with the reconciled reset (the later of the banner and `quota-axi`'s five-hour `resetsAt`, said so in the record), and `bin/fm-limit-resume.sh` is the single tokenless resume owner, scheduled outside the primary so it fires while the primary itself is parked.
A parked worker is a declared external wait everywhere the fleet reads one: the watcher's `task_declares_wait` admits the record beside a `paused:` line, `bin/fm-crew-state.sh` reports `paused` with the reset, and the away-mode daemon routes it to the pause cadence, so nothing interrupts, relaunches, or tears down a parked worker and nothing wedge-escalates it.
Its first sighting queues one informational `check: usage-limit-park` wake naming the reset; later polls take the declared-wait recheck cadence.
After the reset, with the window reading healthy, each parked worker receives exactly one resume steer per park episode through `bin/fm-send.sh`, and the resumed receipt keeps a later sweep from ringing again.
A worker still rendering the same banner after that reset has passed is re-reconciled against `quota-axi`, and a live window that still reads exhausted with a later reset becomes a new episode with its own single steer, so a refused resume never sticks for the life of the banner.
A record with no reconciled reset resumes on a healthy live `quota-axi` read alone, because once the park window has reset the live `resetsAt` already names the next window.
A worker that parked while no watcher was alive to record it has no record once the window resets, so the sweep also resumes a five-hour banner with no record when the banner's own reset has already passed and the live window reads healthy, opening the record and its receipt for that passed reset; `bin/fm-limit-resume.sh`'s header owns that stale-banner condition and its bounds.
A banner naming the weekly limit is a declared wait the sweep never resumes, and a banner headline above Claude's live busy footer is a worker mid-turn, not a park.
The primary is resumed through the pane locked session start recorded: one `usage-window-reset` operational input typed through the same guarded path the away-mode daemon uses, and only into an empty, idle, still-alive session; a primary outside any reachable pane instead finds a durable `check: usage-window-reset` wake on its next turn, so the pending wake is presented either way.
The durable wake queue already preserves every actionable record across the outage; what the park adds is the truthful description: the sweep writes `state/.limit-park-outage` when it sees the primary parked or a worker parked behind a stale beacon (never for a park first seen after its own reset, the already-over rule `bin/fm-limit-park-lib.sh`'s header owns), and `bin/fm-guard.sh` describes a stale beacon inside that window as parked on the usage limit from one time to another, instead of alarming about a lapsed watcher, until a healthy watcher is seen again or the window plus its grace has passed.
Every existing boundary holds: no force, no teardown, no discard, no model-issued arm, no shell `&`; the next turn end after the reset re-arms supervision through the ordinary harness protocol.
...
`tests/fm-limit-resume.test.sh` covers the usage-limit outage above with the live 2026-09-04 banner as its positive control and the same pane without it as the negative control: the shape, the record and its reset cross-check, the paused-class verdict in crew state, the watcher, and the daemon, exactly one resume steer after a passed reset and none before it or on a second sweep, a second steer only once a same-banner refresh sees the live reset move and pass, a weekly park that is never steered, an unknown reset that resumes on a healthy live window, a stale banner with no record that is resumed once after its reset passed with an exhausted-window positive control beside it, a headline above a busy footer that is not a park, the guarded primary delivery and its composer deferral, the durable wake and bootstrap line for an unreachable primary, idempotent scheduler installs whose crontab entry stays short under an 8 KB host PATH and whose install and status output names the scheduler and the skipped-systemd reason, a refused crontab entry reported with its length, and the guard's parked-outage notice.

...
- `LIMIT_RESUME: not armed - ...` - the tokenless usage-limit resume sweep is not scheduled in this home, so parked Claude work would wait for a captain message after the window resets; ask the captain once, then run `bin/fm-limit-resume.sh install` (idempotent; a systemd user timer or the crontab), or write `off` to `config/limit-resume` if they decline.
- `LIMIT_RESUME: this session cannot resume itself ...` - the primary was launched outside a tmux or herdr pane the sweep can type into; workers still resume and a durable wake waits for the next message, so relay the line's launch guidance (start firstmate inside tmux) to the captain once and continue.
